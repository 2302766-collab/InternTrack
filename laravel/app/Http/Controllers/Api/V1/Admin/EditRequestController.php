...
class EditRequestController extends Controller
{
    ...
    private function applyDtrChanges(EditRequest $editRequest): void
    {
        ...
        $timeIn = Carbon::parse($changes['time_in_at']);
        $lunchOut = Carbon::parse($changes['lunch_out_at']);
        $lunchIn = Carbon::parse($changes['lunch_in_at']);
        $timeOut = Carbon::parse($changes['time_out_at']);

        $firstMinutes = max(0, $timeIn->diffInMinutes($lunchOut));
        $secondMinutes = max(0, $lunchIn->diffInMinutes($timeOut));
